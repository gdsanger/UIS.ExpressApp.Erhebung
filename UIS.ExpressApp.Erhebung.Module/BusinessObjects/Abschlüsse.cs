...
namespace UIS.ExpressApp.Erhebung.Module.BusinessObjects
{
    ...
    [XafDisplayName("Abschlüsse")]
    [XafDefaultProperty("MatchKey")]
    [CreatableItem(false)]
    public class Abschlüsse : XPLiteObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public Abschlüsse(Session session)
            : base(session)
        {
        }
        ...
        public void ImportData(ImportArgs args)
        {
            ...
            foreach (string Line in Lines)
            {
                if (Line != "")
                {
                    ...
                    Abschlüsse h = Session.FindObject<Abschlüsse>(new BinaryOperator("Key", Key));
                    if (h == null)
                    {
                        h = new Abschlüsse(Session);
                        h.Key = Key;
                    }
                    h.Name = Name;
                    Session.Save(h);
